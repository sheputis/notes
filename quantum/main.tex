...
Note: the following paragraph has some ontological problems, take it with a grain of salt, it will be revised.
So Bob's positron can be in spin up state with probability $|a|^2$ and down state with
probability $|b|^2$. An interesting observation to make is that this is a mixed
state. Meaning, regarded in isolation on planet B, the positron is still in either  up or down
state quantum mechanically, but it is still described by a density matrix that would look exactly
the same if we actually simply did not know what the state is and had a purely statistical description
that says that it is already in one of the states and here are the probabilities. However the distinction
has to be made that quantum mechanically the spin is still either up or down, but since it is not locally
in a superposition, but implicitly in a superposition with the electron on planet B, the reduced matrix
description encodes in the sense that locally on planet B the measurements on the electron will behave
as if it is statistically in either state.
\end{document}
